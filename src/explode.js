...
import { EXPLOSION_RADIUS, PARTICLE_COUNT, REF_HZ } from './config.js';
...
export function explode(x, y) {
  ...
  const force = 6 * REF_HZ;  // px/sec (was 6 px/tick)
  if (state.player) applyRadialKnockback(state.player, x, y, R, force, 40, state.godMode);
  for (const e of state.enemies) {
    if (e.hp <= 0) continue;
    applyRadialKnockback(e, x, y, R, force, 40, false);
  }
